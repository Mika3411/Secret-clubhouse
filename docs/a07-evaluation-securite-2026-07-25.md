...
| Contrôle | Résultat |
|---|---|
| `npm test` | 213 tests : 208 réussis, 0 échec, 5 suites PostgreSQL ignorées faute de `TEST_DATABASE_URL` locale |
...
| `android/gradlew assembleDebug` | APK 1.10 (`versionCode 11`) construit, synchronisé avec le build web et copié dans `public/downloads/Secret-Clubhouse.apk` |
...
La version 1.10 (`versionCode 11`) est publiée dans `public/downloads/Secret-Clubhouse.apk` et proposée uniquement depuis le groupe parent « Compte et application ». Elle conserve l’identité de signature de test des APK 1.6 à 1.9 afin de rester installable comme mise à jour sur les appareils du prototype. Le flux `mobile:sync` retire l’APK public du build web avant la copie Capacitor afin qu’un paquet Android n’embarque jamais le paquet de téléchargement précédent.
