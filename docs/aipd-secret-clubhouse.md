...
| A07 | **Rouverte** | Rapport historique ; nouvelle évaluation locale du 25 juillet ; suite Node, audit npm, build web, lint Android et APK 1.9 public | Les quatre erreurs Android trouvées ont été corrigées, mais les essais réels RTC, Web Push, FCM, APNs/PushKit et appareils verrouillés restent nécessaires ; l’APK conserve encore l’identité de signature Android de test |
...
Le paquet Android public est ensuite mis à jour en version 1.9 (`versionCode 10`) avec le client web vérifié, le lancement direct des parties multijoueurs acceptées et l’arrêt confirmé d’une partie enregistré pour ses deux participants. Cette reconstruction ne modifie ni les finalités, ni les prestataires, ni les transferts, ni les durées de conservation. Elle conserve volontairement l’identité de signature de test pour permettre la mise à jour des appareils du prototype ; le constat A07 relatif à cette identité reste donc élevé et ouvert.
...
Cette clôture historique valait uniquement tant que `RTC_ENABLED`, `WEB_PUSH_ENABLED`, `NATIVE_PUSH_ENABLED` et `PRIVACY_ADMIN_ENABLED` restaient à `false` et qu’aucun APK/AAB/IPA n’était distribué. Les activations RTC, Web Push, FCM et APNs, puis la publication initiale de l’APK 1.7 désormais mis à jour en 1.9, rouvrent donc `A07`. Les fiches D2 à D5, les tests de garde, un essai d’appel entre deux comptes, des essais de notification consentie sur appareils verrouillés et une signature Android de distribution protégée sont des entrées de la nouvelle évaluation, mais ne suffisent pas seuls à la fermer.
