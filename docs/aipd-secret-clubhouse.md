...
| A07 | **Rouverte** | Rapport historique ; nouvelle évaluation locale du 25 juillet ; suite Node, audit npm, build web, lint Android et APK 1.11 public | Les quatre erreurs Android trouvées ont été corrigées, mais les essais réels RTC, Web Push, FCM, APNs/PushKit et appareils verrouillés restent nécessaires ; l’APK conserve encore l’identité de signature Android de test |
...
| R02 | Limitation, cookie/Bearer, révocation et erreurs évalués ; A07 et A08 ouvertes | 2 → 3 | Le code et le Blueprint imposent 12 h et les assets publics correspondent à la version évaluée, mais aucune mesure d’une session authentifiée réelle ni capture expurgée de la variable Render ne prouve encore cette durée | 4×3=12 — **Élevé** |
...
Le paquet Android public est ensuite mis à jour en version 1.11 (`versionCode 12`) avec le client web vérifié, le lancement direct des parties multijoueurs acceptées, l’arrêt confirmé d’une partie enregistré pour ses deux participants et l’ouverture sûre des URL `http://` et `https://` présentes dans les messages enfant ou parent. Après cet arrêt, la carte d’invitation disparaît des conversations des deux participants tandis que l’état technique annulé reste soumis à la rétention des parties. Les autres contenus de message restent du texte et ne sont jamais interprétés comme du HTML. Le code de version supérieur permet à Android de reconnaître réellement cette reconstruction comme une mise à jour des APK 1.6 à 1.10. Cette reconstruction ne modifie ni les finalités, ni les prestataires, ni les transferts, ni les durées de conservation. Elle conserve volontairement l’identité de signature de test pour permettre la mise à jour des appareils du prototype ; le constat A07 relatif à cette identité reste donc élevé et ouvert.
...
Cette clôture historique valait uniquement tant que `RTC_ENABLED`, `WEB_PUSH_ENABLED`, `NATIVE_PUSH_ENABLED` et `PRIVACY_ADMIN_ENABLED` restaient à `false` et qu’aucun APK/AAB/IPA n’était distribué. Les activations RTC, Web Push, FCM et APNs, puis la publication initiale de l’APK 1.7 désormais mis à jour en 1.11, rouvrent donc `A07`. Les fiches D2 à D5, les tests de garde, un essai d’appel entre deux comptes, des essais de notification consentie sur appareils verrouillés et une signature Android de distribution protégée sont des entrées de la nouvelle évaluation, mais ne suffisent pas seuls à la fermer.
