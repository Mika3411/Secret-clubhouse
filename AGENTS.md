...
- Windows users receive standards-based Web Push through Chrome, Edge, or the installed PWA. Notification settings expose only the Windows-specific activation state; clicking a message notification opens its conversation. Stable VAPID keys come from Render environment variables when configured and otherwise persist in Render PostgreSQL.
