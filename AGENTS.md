...
- The child profile uses one centered 300 px card column: contact ID, protection, notifications, parent access, preferences, and sign-out share consistent widths, radii, icon proportions, and spacing on phones and tablets; the indigo background continues without a light gap down to the fixed navigation.
...
- Windows users receive standards-based Web Push through Chrome, Edge, or the installed PWA. Notification settings expose a Windows-specific activation state and an end-to-end test action; clicking a message notification opens its conversation. Stable VAPID keys come from Render environment variables when configured and otherwise persist in Render PostgreSQL.
